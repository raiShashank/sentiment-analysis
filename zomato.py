...
class Zomato(object):
    def __init__(self, url):
        ...
        print self.url
        self.soup = self.alt_get_soup()
        #print self.soup
        self.res_id = self.get_res_id()

    #Wrote an alternate function to get the source code
    def alt_get_soup(self):
        import urllib2
        html = urllib2.urlopen(self.url)
        page = html.read()
        html.close()
        return BeautifulSoup(str(page))

    #The below function is returing Error code 500 due to requests.get
    def get_soup(self):
        print HEADERS
    ...
    #original function to extractreviews
    ...
    def get_date_review(self):
        spans = self.soup.findAll('time', {'itemprop':'dtreviewed'})
        if not spans:
            return []
        return [item.contents[0].strip() for item in spans]
        # returns the text that appears on webpage
        #But item.get('datetime') returns the exact date and time when the review
        #was written

    def get_date_and_reviews(self):
        spans = self.soup.findAll('time', {'itemprop':'dtreviewed'})
        if not spans:
            return []
        all_itemprops = self.soup.findAll('div', {'itemprop':'description'})
        reviews = [item.contents[1].contents for item in all_itemprops]
        reviews = [''.join([item for item in review if type(item).__name__=='NavigableString']).strip() for review in reviews]
        dates = [item.get('datetime') for item in spans]
        return [x for x in zip(dates, reviews)]

    #function to test various aspects of reviews
    def test_reviews(self):
        #print self.soup.findAll()[0]
        #print type(self.soup.findAll())
        #print len(self.soup.findAll())
        spans = self.soup.findAll('div', {'itemprop':'description'})
        #Both of these 2 will do the same thing        
        all_itemprops = filter(lambda item:item.get('itemprop')=='description', self.soup.findAll())
        # print spans
        # print
        # print all_itemprops

        # print type(all_itemprops)
        # print len(all_itemprops)    
        # for item in all_itemprops : 
        #     print item
        #     print "shashank"
        #     print item.contents[1].contents
        #     print "Prabhansh"
...
if __name__ == '__main__':
    ...
    for url in urls:
        ...
        #print z.get_menu()
        #z.test_reviews()
        #print z.get_date_review()
        # for r in z.test_reviews():
        #     print r.encode('ascii', 'ignore')
        #     print

        for r in z.get_date_and_reviews():
            print r[0]
            print r[1].encode('ascii', 'ignore')
            print
        #print z.get_cover()
